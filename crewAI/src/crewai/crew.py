...
from crewAI.src.crewai.utilities import Logger
...
from crewAI.src.crewai.agent import Agent
from crewAI.src.crewai.agents.cache import CacheHandler
from crewAI.src.crewai.process import Process
from crewAI.src.crewai.task import Task
from crewAI.src.crewai.tools.agent_tools import AgentTools
from crewAI.src.crewai.utilities import I18N, Logger, RPMController
...
class Crew(BaseModel):
    ...
    def kickoff(self) -> str:
        ...
        if self.process == Process.sequential:
            task_ouput,task_output_list = self._run_sequential_process()
            ...
            with open("combined_ouptut.json", 'w') as json_file:
                json_file.write(json.dumps({'Task_output_list': task_output_list}) + '\n')
    ...
    def _run_sequential_process(self) -> str:
        ...
        for task in self.tasks:
            ...
            logger = Logger(verbose_level=2, log_folder='logs', log_filename='example.json')
            ...
            task_output_list.append({f"{role}":task_output})
            

        if self.max_rpm:
            self._rpm_controller.stop_rpm_counter()

        return task_output,task_output_list
