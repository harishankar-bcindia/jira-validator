...
def crew_agent(ticket_detail):    #ticket detail is the df of each row.
    ...
    task_description = f''' You have to fill complete raw json format provided based on your analysis of JIRA ticket.
                ...
                Analyse above ticket details, But before filling raw json you have to analyze below things carefully in ticket and do not write any comment in your output and return only filled raw json, do not delete or add any extra key:
                            ...
                            Instruction 8: After overall analysis and filling of values to the corresponding keys now return complete filled json but do not delete any key of json even if it is empty.Do not Mention anything with HTML Tags markdown. And do not add any characters like //,#,##,\\n or \.
...
# Function to extract URL from HTML string
...
# df['url'] = df['ticket_link'].apply(extract_url)
...
def call_crew(df):
    ...
    for index, row in df.iterrows():
        ...
        # output_df['Ticket link'] = output_df.apply(lambda x: make_clickable(x['url'], x['Ticket Id']), axis=1)
        # output_df['Ticket link'] = output_df.apply(lambda x: make_clickable(x['url'], x['Ticket Id']), axis=1)

        # from IPython.display import display, HTML

        # # Assuming output_df['Ticket link'] contains the HTML strings
        # html_links = output_df['Ticket link'].tolist()

        # # Join the HTML strings and display as HTML
        ...
        # clickable_links = []
        # for link in output_df['Ticket link']:
        #     clickable_links.append(link)
        # column_to_shift = output_df['Ticket_link']
        # desired_position = 1
        # column_values = output_df[column_to_shift]
        # output_df.drop(columns=[column_to_shift], inplace=True)


    # output_df.to_excel('Output.xlsx',index=False)
    output_df_list.append(output_df.to_dict(orient="records")[0])
    # return output_df